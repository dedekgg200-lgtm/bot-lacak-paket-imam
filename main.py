...
def status_pembayaran(data):
    """
    Membaca informasi COD/NONCOD hanya jika memang
    dikirim oleh API. Tidak menebak dari status paket.
    """
    ...
    def cari(obj):
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                ...
                nama = str(key).lower().strip()
                teks = "" if value is None else str(value).strip()

                if teks and any(k in nama for k in [
                    "service",
                    "payment",
                    "cod"
                ]):
                    ditemukan.append((nama, teks.upper()))

                # Telusuri struktur JSON yang lebih dalam
    ...
    # NONCOD harus diperiksa lebih dahulu agar
    # kata NONCOD tidak terbaca sebagai COD.
    for nama, nilai in ditemukan:
    ...
    # COD yang benar-benar tertulis sebagai COD
    for nama, nilai in ditemukan:
    ...
    for nama, nilai in ditemukan:
        ...
        if any(kata in nilai for kata in [
            "PAID",
            "SUDAH DIBAYAR",
            "SUDAH BAYAR",
            "LUNAS",
...
def lacak_resi(resi):
    ...
    try:
        ...
        asal = (
            ambil_field(summary, "origin")
            or ambil_field(detail, "origin")
            or "Tidak tersedia"
        )

        tujuan = (
            ambil_field(summary, "destination")
            or ambil_field(detail, "destination")
            or "Tidak tersedia"
        )

        status_asli = (
            ambil_field(summary, "status")
            or ambil_field(detail, "status")
            or "Tidak tersedia"
        )

        tanggal = (
            ambil_field(summary, "date", "datetime")
            or ambil_field(detail, "date", "datetime")
            or "Tidak tersedia"
        )
